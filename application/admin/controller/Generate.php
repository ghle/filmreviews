...
class Generate extends Controller
{
    ...
    public function run()
    {
        ...
        public function category($data)
        {
           ...
           $ds['catname']=$data['title'];
           $ds['catcon']=$data['controller'];
           $res=db('category')->insert($ds);
        }
        ...
        if (isset($this->data['form']) && $this->data['form']) {
            foreach ($this->data['form'] as $form) {
                ...
                if (!in_array($form['name'], ['id', 'isdelete', 'create_time', 'update_time'])) {
                    ...
                    $editField .= tab(2) . '<div class="layui-form-item">' . "\n"
                        . tab(3) . ' <label for="username" class="layui-form-label">'
                        . (isset($form['require']) && $form['require'] ? '<span class="x-red">*</span>' : '')
                        . $form['title'] . '：</label>' . "\n"
                        . tab(3) . '<div class="layui-input-inline"'
                    ...
                    switch ($form['type']) {
                        ...
                        case "number":
                        default:
                            $editField .= tab(4) . '<input type="' . $form['type'] . '" class="layui-input" class="layui-input" required="" lay-verify="required" id="username"'
                                . 'placeholder="' . $form['title'] . '" name="' . $form['name'] . '" '
                                . 'value="' . '{$vo.' . $form['name'] . ' ?? \'' . $form['default'] . '\'}' . '" '
                                . $validateForm . '>' . "\n";
                            break;
                    ...
                    $editField .= tab(3) . '</div>' . "\n"
                        . tab(3) . ' <div class="layui-form-mid layui-word-aux">' . "\n"
                        .tab(5).' <span class="x-red">*</span>'."\n"
                        . tab(3) . '</div>' . "\n"
                        .tab(2).'</div>'."\n";
                }
            }
